...
| 03 | sid | uint64_t | 8 |会话ID|每个连接的会话ID都不一样|
| 04 | cid | uint64_t | 8 |连接ID| |
| 05 | nid | uint32_t | 4 |结点ID| |
| 06 | ctm | uint64_t | 8 |连接时间|精确到秒|
| 07 | seq | uint64_t | 8 |流水号| |
| 08 | body | char[] | 0 |消息体|各协议报体内容, 紧接在协议头后|
...
>{<br>
>   required uint64 sid = 1;        // M|会话ID|数字|<br>
